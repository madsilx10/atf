...
from pyrogram.raw.functions.messages import RequestAppWebView, StartBot
    ...
    async with client:
        ...
        try:
            ...
            if "BOT_APP_INVALID" in str(e):
                # akun ini kemungkinan belum pernah start bot, start dulu (bawa ref param)
                try:
                    if hasattr(bot_peer, "user_id"):
                        bot_input_user = InputUser(
                            user_id=bot_peer.user_id,
                            access_hash=bot_peer.access_hash,
                        )
                    else:
                        bot_input_user = bot_peer
                    await client.invoke(
                        StartBot(
                            bot=bot_input_user,
                            peer=bot_peer,
