"""
ATF Miners Bot - Multi Account  v1.3.4
...
class ATFSession:
    ...
    def do_task(self, task_id: str):
        ...
        delay = random.uniform(TASK_START_DELAY_MIN, TASK_START_DELAY_MAX)
        time.sleep(delay)
...
def run_account(idx, session_str, mnemonic_str, mode):
    ...
    if mode == "full" and not wallet_verified:
        ...
        for attempt in range(1, 9):
            ...
            if status != "busy":
                break
            print("  wallet : busy, tunggu 15 detik...")
            time.sleep(15)
            nonce = s.get_proof_payload(force=True)
