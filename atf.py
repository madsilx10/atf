...
def run_account(idx, session_str, mnemonic_str, mode):
    ...
    # claim (mode terpisah: cek status mining, lalu claim kalau ada saldo)
    if mode == "claim":
        try:
            r_status = s.start_mine()
            u = r_status.get("user", {}) if isinstance(r_status, dict) else {}
            preview = u.get("claimable_now") or r_status.get("pending_reward") or r_status.get("claim_preview") or 0
