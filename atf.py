...
class ATFSession:
    ...
    def request_withdraw(self, amount: float) -> dict:
        return self._post("request_withdraw", self._p({
            "amount": str(amount),
        }))
...
def run_account(idx, session_str, mnemonic_str, mode, mode_extra=None):
    ...
    if mode == "withdraw":
        try:
            amt = mode_extra.get("amount", 0)
            if amt <= 0:
                print("  withdraw: amount tidak valid, skip")
            else:
                r = s.request_withdraw(amt)
                print(f"  withdraw: {r.get('status')} | {r.get('message','')}")
        except Exception as e:
...
def main():
    ...
    if mode == "withdraw":
        while True:
            try:
                amt = float(input("Amount withdraw (misal: 1000): ").strip())
                if amt > 0:
                    mode_extra["amount"] = amt
                    break
            except (ValueError, KeyboardInterrupt):
                pass
            print("  Input tidak valid.")
